...
export default function AbmPage() {
  ...
  async function loadAlumni(query: string) {
    try {
      setLoading(true);
      const params = new URLSearchParams({ strand: "ABM" });
      if (query.trim()) {
      ...
      console.error(err);
      setError("Could not load ABM alumni list.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6 py-12">
        <div className="w-full max-w-3xl rounded-2xl border-2 border-[#B23B3B] bg-white/95 backdrop-blur-sm p-8 shadow-xl shadow-brand-lg animate-fade-in">
          <h2 className="mb-6 text-center text-2xl font-bold text-[#B23B3B] text-shadow">
            ABM — Alumni List & Contact Information
          </h2>
          ...
          {loading && (
            <p className="text-center text-[#A03E2D]">Loading ABM alumni...</p>
          )}
